    ...
    <div class="card-body">
      ...
      <ul class="nav nav-tabs" id="myTab" role="tablist">
        <li class="nav-item" role="presentation">
          <button class="nav-link active" id="home-tab" data-bs-toggle="tab" data-bs-target="#home" type="button" role="tab" aria-controls="home" aria-selected="true">بيانات المرفق</button>
        </li>
        <li class="nav-item" role="presentation">
          <button class="nav-link" id="profile-tab" data-bs-toggle="tab" data-bs-target="#profile" type="button" role="tab" aria-controls="profile" aria-selected="false" tabindex="-1">صورة المرفق</button>
        </li>
        <li class="nav-item" role="presentation">
          <button class="nav-link" id="contact-tab" data-bs-toggle="tab" data-bs-target="#contact" type="button" role="tab" aria-controls="contact" aria-selected="false" tabindex="-1">التقييمات و التعليقات</button>
        </li>
      </ul>
                            ...
                            <div class="col-2">
                                ...
                                @if (auth()->user()->role == 'مسؤول نظام')
                                    <form action="{{ route('reating.destroy', $rating->id) }}" method="POST">
                                        @method('DELETE')
                                        @csrf
                                        <button role="submit" class="btn btn-danger">حذف <i class="bi bi-send-x"></i></button>
                                    </form>
                                @endif
                            </div>
                        </div>
